...
| T-05 | Parcial | `PolicyController` expone lectura scoped, drafts, publicación+activación atómica, retiro y simulación no persistente; falta evaluación real de snapshot y edición versionada completa. `dotnet build` de Api correcto. | `3bee30a` / Bloque 3 en curso |
| T-06 | Parcial | Se agregó `PolicyFactRequest`, registry exact-one local, timeout 5 s, manifest de líneas y `EvaluateEnterprisePurchaseRequest` que carga la política activa desde persistencia mediante parser canónico. Faltan catálogos completos y manifest de sourcing/attestation exhaustivo. | `3bee30a` / Bloque 3 en curso |
