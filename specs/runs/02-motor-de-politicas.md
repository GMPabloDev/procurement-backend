...
| T-04 | Parcial | `PolicyPersistenceService`: selección serializable de activación, retiro append-only, auditoría administrativa, rowversion e idempotencia por SHA-256; reserva persistente `PolicyEvaluationReservations` con índice scoped y lease de 30 s antes del provider; integración SQL Server `PolicyPersistenceServiceTests` verifica reserva/liberación y espera concurrente desde otro DbContext; `CountingFactProvider` verifica una adquisición, replay sin provider y conflicto de fingerprint previo al provider. Append de evaluación conserva `evaluation_sequence` y `evaluation_result_digest`; aún falta cierre atómico de sucesor y diff/reevaluación completo. | `568f1a6` / Bloque 2 en curso |
...
| T-07 | Parcial | `QuotationWaiverEvaluator` limita la excepción, valida floor contra el control publicado, binding/nonce/evidence, autoridad y SoD; `ApplyVerifiedQuotationWaiver` actualiza controles y scopes y recalcula el digest canónico. Falta persistir snapshot/replay y verifier registry de workflow. | `c7dd91a` / Bloque 3 en curso |
| T-08 | Parcial | `PolicyConfigurationHealthCheck` y `/health/policy` distinguen ausencia/ambigüedad y validan estado publicado + digest SHA-256 del contenido cargado sin exponer reglas; `ApiE2ETests` verifica `503` + `POLICY_CONFIGURATION_REQUIRED`; faltan corrupción/indisponibilidad HTTP. | `82283af` / Bloque 4 en curso |
| T-09 | Parcial | Unitarias: 31 correctas; IntegrationTests: 8 correctas con SQL Server/Testcontainers; ApiE2ETests: 2 correctas (incluye `/health/policy` required); solución compila con 0 advertencias/errores; `lens_diagnostics --mode=all --severity=error`: sin errores. Se agregó replay real con provider instrumentado, corrupción fail-closed y metadata de secuencia/digest. Falta ampliar evidencia negativa/golden/API específica de CA-01–CA-12. | `82283af` / Bloque 4 |
