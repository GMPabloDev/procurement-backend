...
| T-05 | Parcial | `PolicyController` expone lectura, drafts, publicación como nueva versión, activación, retiro y simulación no persistente; falta completar validación tipada y simulación real del contrato. `dotnet build` de Api correcto. | working-tree / Bloque 3 en curso |
| T-06 | Parcial | Se agregó `PolicyFactRequest`, manifest/provider registry exacto, timeout de 5 s y `EvaluateEnterprisePurchaseRequest`; workload allowlist y selección activa validan organización/digest. Faltan catálogos completos y manifest de sourcing/attestation exhaustivo. | working-tree / Bloque 3 en curso |
