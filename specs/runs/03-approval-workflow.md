...
- Tests y checks: build `--no-restore` **0 avisos / 0 errores**; UnitTests **99/99**; IntegrationTests **37/37** (incluye `V2_preflight_blocks_a_legacy_event_baseline` y `Reconciliation_lease_is_fenced_and_reclaimed_reusing_run_root_and_cursor`); ApiE2ETests **12/12**; `specctl run-lint 03` ✅; `git diff --check` limpio.
