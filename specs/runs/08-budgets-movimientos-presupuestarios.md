...
> **Actualizado:** 2026-09-15 09:35 -0500
...
| T-05 | Verificada | Owner real `budget-check-owner/v1`: `BudgetPrerequisiteProcessor` con attempt durable, keys deterministas, lease/fencing con renovación máxima cada 10 s (`HoldLeaseAsync`) y comprobación de lease antes de cada movimiento o checkpoint, reserva all-or-nothing y señal con `budget-check-evidence/v1`; prerequisito `FAILED` sin reserva cuando falta saldo. La revisión independiente detectó que el lease no se renovaba y que un fallo tras confirmar la operación no liberaba el lease: corregido en CP-05. Evidencia: `Budget_control_reserves_all_or_nothing_and_signals_the_prerequisite`, `A_budget_without_funds_fails_the_owner_prerequisite_without_reserving`, `An_expired_lease_of_a_dead_worker_is_reclaimed_without_double_booking` y `A_crash_after_the_confirmed_signal_is_finished_from_the_recorded_signal`. | working-tree / CP-05 |
...
- Cambios: `BudgetPrerequisiteProcessor` reclama attempts no terminales aunque el prerequisito ya haya decidido, resuelve la señal registrada antes de compensar, renueva y verifica el lease (`HoldLeaseAsync`, ≤10 s) antes de cada movimiento/checkpoint y registra el fallo posterior a una operación confirmada conservando estado y liberando lease; `BudgetPersistenceService.RebuildAsync` reconstruye como allocation current + deltas y `BudgetHealthCheck` compara contra esa reconstrucción; tests nuevos de CA-01 (carrera y reducción/refs inválidas), CA-02 (lote multi-posición), CA-03 (agrupación), CA-06 (lease vencido y crash tras señal), CA-07 (golden de release) y CA-08 (health tras revisión de allocation).
- Tests y checks: build 0 errores; Unit `200/200`; Integración `103/103`; API/E2E `27/27`; `git diff --check` limpio; `specctl run-lint 08` válido. Suites completas ejecutadas una vez sobre el árbol estable de este checkpoint.
- Evidencia: `Two_concurrent_allocation_revisions_leave_one_current_version`, `An_allocation_reduction_below_held_funds_and_invalid_references_are_rejected`, `A_multi_position_batch_with_one_failing_movement_confirms_nothing`, `A_precheck_groups_targets_per_position_and_never_changes_buckets`, `An_expired_lease_of_a_dead_worker_is_reclaimed_without_double_booking`, `A_crash_after_the_confirmed_signal_is_finished_from_the_recorded_signal`, `Release_fingerprint_is_reproducible_from_the_contract_fixture`, `Health_rebuilds_the_projection_instead_of_reading_a_revision_as_corruption`.
- HEAD: `working-tree sobre 329e842` (código pendiente de commit).
- Próximo paso: commit del bloque y ronda diferencial de revisión.
...
| CA-03 | Cumplido | `A_precheck_groups_targets_per_position_and_never_changes_buckets` agrupa dos líneas de una posición y una de otra, crea un `REQUESTED` por target, no toca buckets y no reserva parcialmente ante insuficiencia; el precheck por referencia y su respuesta minimizada se cubren por E2E. | Revisión pendiente |
...
| CA-06 | Cumplido | Release automático probado end-to-end (rechazo → REVERSE → RELEASE idempotente) y release previa a la cancelación de PR implementada con attempt durable. Transferencia atómica, release automático, compensación y cancelación de PR probados sobre SQL real. El lease se renueva cada ≤10 s con fencing y comprobación de titularidad antes de cada movimiento/checkpoint, `An_expired_lease_of_a_dead_worker_is_reclaimed_without_double_booking` recupera un lease vencido sin doble reserva y `A_crash_after_the_confirmed_signal_is_finished_from_the_recorded_signal` cierra un crash entre señal y checkpoint consultando la señal registrada. | Revisión pendiente |
...
> **Resultado:** BLOCK (ronda 1); correcciones aplicadas, ronda diferencial pendiente.
> **Rondas:** 1/2
> **Triaje:** R8/R9 (recuperación de una señal confirmada, renovación y fencing del lease) corregidos con regresión; R10 (health falso `BUDGET_LEDGER_CORRUPTED` tras revisión de allocation) corregido con regresión; R11 (CA-01/CA-02/CA-03/CA-07 sin evidencia automática) cerrado con pruebas dedicadas. Ningún hallazgo descartado.
> **Modelo efectivo:** `openai-codex/gpt-5.6-sol` (subagente `sdd-implementation-reviewer`, effort high; 8 turnos).
> **Método:** revisión full de la base `534a7c83b42f2727a143298d77b2a2fd47d0536d` al candidato `329e842`, contrastando contrato, código y evidencia ejecutada del run; el revisor confirmó identidad Git y no reejecutó suites.
> **Fecha:** 2026-09-15
