...
> **Actualizado:** 2026-09-14 12:33 -0500
...
|---|---|---|---|
| T-01 | Verificada | Dominio `ReferenceCatalogs` (código/versión/digest, proyecciones y canonicalizador `spend-category/v1`), persistencia raíz+versiones append-only con `predecessor_version`, audit atómico y migración `Spec07ReferenceCatalogs`. Evidencia: `ReferenceCatalogCanonicalizationTests` (3), `ReferenceCatalogPersistenceTests` (7: versiones/audit, unicidad case-insensitive, carrera, bloqueo por scope, digest por versión, Department current, catálogos Policy/resolver). | working-tree sobre `40e6678e` / CP-01–CP-02 |
...
| T-05 | Verificada | Seis slots owner declarados explícitamente, `ReferenceType` no anulable sin wildcard; owners reales Cost Center (actividad y relación exacta) y Spend Category (código/versión/digest). Evidencia: `PurchaseRequestReferenceOwnerRegistryTests` (2), `ReferenceCatalogOwnerTests` (3). | working-tree sobre `40e6678e` / CP-04 |
| T-06 | Verificada | Attestation valida eco, `owner_id`, `owner_contract_version` y coherencia `active`/`status`; submit real con owners y catálogos de producción (sin doubles) atestigua, evalúa Policy, abre caso con scope Cost Center y asigna solo al candidato cubierto; referencias stale fallan cerrado sin manifest ni caso. Evidencia: `ReferenceCatalogSubmitIntegrationTests` (2). | working-tree sobre `40e6678e` / CP-04 |
| T-07 | Verificada | Health por slot/catálogo/storage con gramática exacta sin datos; E2E de cardinalidad 0/2, corrupción de puntero/digest y storage indisponible; runbook `docs/reference-catalogs-operations.md`; regresión completa. Evidencia: `ReferenceCatalogE2ETests.Health_distinguishes_missing_ambiguous_and_corrupted_catalogs`, `PurchaseRequestE2ETests.Health_reports_only_the_really_missing_registrations`. | working-tree sobre `40e6678e` / CP-05 |
...
|---|---|---|---|
| CA-01 | Cumplido | `ReferenceCatalogPersistenceTests` (7) cubre crear/renombrar/reasignar/desactivar/reactivar con historia y audit, unicidad case-insensitive, stale `409`, carrera con una sola sucesora y digest por versión; `ReferenceCatalogCanonicalizationTests` reproduce el SHA-256 `4ed4dc13…` y falla al alterar nombre, organización o versión. | Pendiente de revisión |
...
| CA-04 | Cumplido | `ReferenceCatalogPersistenceTests.Policy_and_approval_resolution_is_organization_bound_and_fail_closed` prueba catálogos Policy válidos, versión stale, otra organización, digest distinto y resolución/`422` de `decision-scope/v1`; `ApprovalContractTests` cubre round-trip/digest con `COST_CENTER`; el E2E de health prueba registros 0 de ambos catálogos. | Pendiente de revisión |
| CA-05 | Cumplido | `PurchaseRequestReferenceOwnerRegistryTests` exige seis slots exactos y rechaza ausencia/ambigüedad; `ReferenceCatalogOwnerTests` verifica `ACTIVE/INACTIVE/NOT_FOUND` por antigüedad, organización, digest y relación exacta Cost Center→Department (incluida versión Department stale). | Pendiente de revisión |
| CA-06 | Cumplido | `ReferenceCatalogSubmitIntegrationTests` (2) ejecuta el recorrido real sin owners ni catálogos permisivos: attestation con owners de catálogo, Policy con catálogos reales, caso con `"dimension":"COST_CENTER"` y asignación solo al candidato cubierto; replay recupera caso y artefactos, y una referencia stale no crea manifest ni caso. | Pendiente de revisión |
| CA-07 | Cumplido | `ReferenceCatalogE2ETests.Health_distinguishes_missing_ambiguous_and_corrupted_catalogs` cubre owner 0/2, catálogos 0, corrupción de puntero/digest y storage indisponible con la gramática exacta y sin datos de negocio; `PurchaseRequestE2ETests` confirma health estable con catálogo vacío y registros reales. | Pendiente de revisión |
| CA-08 | Cumplido | Migración aplicada sobre la línea base en todos los arneses de integración/E2E; scopes y digests históricos se conservan; suites completas Unit **172/172**, Integración **75/75**, API/E2E **24/24**, build 0 errores, `git diff --check` limpio y runbook `docs/reference-catalogs-operations.md` ejercitado por las pruebas de health y desactivación. | Pendiente de revisión |
